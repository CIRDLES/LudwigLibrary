...
public final class SquidMathUtils {
    ...
    /*
     * Function sqWtdAv(Values As Range, Errs As Range, _ Optional PercentErrsIn
     * As Boolean = False, _ Optional PercentErrsOut As Boolean = False, _
     * Optional CanReject As Boolean = False) As Variant ' Calculate an
     * error-weighted average using Isoplot's procedure, ' demanding calculation
     * of an external error if prob-fit is low; ' return an array with the
     * various wtd-avg statistics & headers. Dim b As Boolean, k%, ss$, e$, w As
     * Variant Dim s(1 To 6, 1 To 2) As Variant, ValuesErrs As Range
     *
     * Set ValuesErrs = Union(Values, Errs) w = Isoplot3.wtdav(ValuesErrs,
     * PercentErrsIn, PercentErrsOut, _ 1, CanReject, True, 1) ' W(1,1)= wtd
     * mean internal or external ' W(2,1)= 1-sigma or 68% conf.error ' W(3,1)=
     * required external 1-sigma (or MSWD) ' W(4,1)= MSWD (or #rejected) '
     * W(5,1)= #rejected (or Prob fit) ' W(6,1)= ProbFit ' W(7,1)= rej. item #s
     *
     * If IsNull(w) Then sqWtdAv = "=#ERR": Exit Function Else
     *
     * If IsNumeric(w(3, 1)) Then b = (w(3, 1) <= 0.0001)
     * Else
     * b = True
     * End If
     *
     ...
     * s(5, 2) = "#rejected"
     *
     * If s(5, 1) > 0 Then ss = w(7, 1) Subst ss, " ", ", " s(6, 1) = ss s(6, 2)
     * = "rejected item #s" Else s(6, 1) = "": s(6, 2) = "" End If
     *
     * End If
     *
     * End If sqWtdAv = s End Function
     */
     ...
     * @return 
     */
    public static double[][] sqWtdAv(double[] values, double[] errors){
        return null;
    }
}
